...
# 将数据库中的 ISO 格式时间字符串转换为毫秒时间戳
            messages = []
            for role, content, timestamp_str in cursor.fetchall():
                try:
        # 解析 ISO 格式时间，例如 "2026-04-15T21:30:00.123456"
                    dt = datetime.fromisoformat(timestamp_str)
                    timestamp_ms = int(dt.timestamp() * 1000)
                except:
                     timestamp_ms = 0
                messages.append({"role": role, "content": content, "timestamp": timestamp_ms})
